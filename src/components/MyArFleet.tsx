...
export default function MyArFleet() {
  ...
  const updatePlacementStatus = useCallback((placementId: string, status: Placement['status']) => {
    ...
    // Update the placement in the queue as well
  ...
  // Define functions and assign them to refs
  processPlacementRef.current = async (placement: Placement) => {
    ...
    try {
      ...
      if (newStatus !== placement.status) {
        ...
        // Remove the placement from the queue if no status change
  ...
  checkProviderReadyRef.current = async (placement: Placement) => {
    ...
    try {
      ...
      // Return the new status to ensure it's reflected in the processing loop
      return 'transferring';
    } catch (error) {
